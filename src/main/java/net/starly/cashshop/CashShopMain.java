...
public class CashShopMain extends JavaPlugin {
    ...
    @Getter
    private CashShopRepository cashShopRepository;
    @Getter
    ...
    public void onDisable() {
        if (cashShopRepository != null) cashShopRepository.getShops().forEach((shop) -> shop.save(false));
        STContainer.closeShopAll();
        AsyncExecutors.shutdown();
        ConnectionPoolManager pool = ConnectionPoolManager.getInternalPool();
        if (playerCashRepository != null) playerCashRepository.close();
        if (pool != null) pool.closePool();
    }
