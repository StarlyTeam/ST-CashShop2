...
public abstract class STContainer implements InventoryHolder {
    ...
    private static void registerPlayer(String shopKey, Player player) {
        viewerMap.put(player.getName(), shopKey);
    }

    private static void unregisterPlayer(Player player) {
        viewerMap.remove(player.getName());
    }
    ...
    protected void headItemSetting() {
        ...
        if (setting != null) if (size > setting.getSlot()) {
            ...
            if (inventory.getItem(setting.getSlot()) != null)
                inventory.getItem(setting.getSlot()).setItemMeta(skull.getItemMeta());
