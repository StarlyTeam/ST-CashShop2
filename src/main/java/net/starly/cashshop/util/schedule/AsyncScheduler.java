...
public class AsyncScheduler implements STScheduler {
    ...
    public SchedulerRunnable getTask(int taskId) {
        LinkedList<SchedulerRunnable> list = (LinkedList)this.runnableList.clone();
    ...
    public long heartBeatProcess(long currentTick) {
        ...
        ListIterator it = this.runnableList.listIterator();

        while(it.hasNext()) {
            SchedulerRunnable run = (SchedulerRunnable)it.next();
            if (run.processTickAndCheck(currentTick)) {
                AsyncExecutors.getService().submit(new ExceptionHandle(run));
            }
