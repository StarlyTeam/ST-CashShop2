    ...
    public static final STSubCommand INFO = new STSubCommand("확인", "info", "해당 유저의 캐시를 확인합니다.", playerSubNoMoney, (sender, args)-> {
        ...
        if(target == null) {
            AsyncExecutors.run(()-> {
                ...
                else context.get(MessageContext.Type.DEFAULT, "info", new Replacer.ReplacerBuilder().append(off).append(plugin.getPlayerCashRepository().getPlayerCash(off.getUniqueId()).getCash(), false).build().getFunction()).send(sender);
            });
        } else context.get(MessageContext.Type.DEFAULT, "info", new Replacer.ReplacerBuilder().append(target).append(plugin.getPlayerCashRepository().getPlayerCash(target.getUniqueId()).getCash(), false).build().getFunction()).send(sender);
    });
